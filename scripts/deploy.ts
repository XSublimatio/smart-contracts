...
async function main() {
    ...
    const proceedsDestination = process.env.PROCEEDS_DESTINATION ?? 10 * 86400;
    ...
    console.log(`Proceeds Destination: ${proceedsDestination}`);
    ...
    const contract = await (
        ...
        ).deploy(baseUri, deployerAddress, proceedsDestination, pricePerTokenMint, launchTimestamp)
    ).deployed();
